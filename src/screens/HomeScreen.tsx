...
const HomeScreen = ({ navigation, route }: Props) => {
  ...
  const fetchUser = async () => {
    const deviceId =
      (await AsyncStorage.getItem("deviceId")) ?? DeviceInfo.getUniqueIdSync();
  ...
  return (
    <SafeAreaView
      className="flex-1 bg-[#FAFAFA] justify-end"
      edges={["left", "right"]}
    >
      {loading && (
        <Animated.View
          entering={FadeIn}
          className={
            "absolute w-full h-full z-30 flex-1 items-center justify-center bg-[#00000099]"
          }
        >
      ...
      {typeof currentUser !== undefined && (
        <>
          <Animated.View
            className="bg-[#4CAF50] absolute top-0 z-20 rounded-b-3xl h-1/6 w-full justify-center px-5"
            style={[
          ...
          <FlashList
            ...
            renderItem={({ item, index }) => (
              <NoteItem note={item} key={index} index={index} />
            )}
            ListEmptyComponent={() =>
              loading ? (
                <></>
          ...
          <Animated.View
            className={
              "w-16 h-16 items-center justify-center absolute right-7 bottom-12"
            }
            style={[
              commonStyles.largeBottomShadow,
              { backgroundColor: "#4CAF50", borderRadius: 32, elevation: 15 },
            ]}
          ...
          >
            ...
            >
              <Ionicons name="add-sharp" size={35} color={"#FFFFFF"} />
            </TouchableOpacity>
          </Animated.View>
        </>
      )}
      ...
      >
        <View style={styles.centeredView}>
          ...
          >
            <Text
              className="font-semibold text-xl text-center mx-5"
              style={{ color: "#212121", fontFamily: "Inter", fontSize: 22 }}
            >
              {t("ratingSentence")}
            </Text>
            ...
            <TouchableOpacity
              className="w-1/3 items-center rounded-xl mt-2 py-1 justify-center"
